...
public class ListaLSE {
	...
	private static void imprimirAoContrario(No no) {
		No aux = no;
		No aux1 = null;
		/**
	...
	private static int contar(No inicio) {
		int cont = 0;
		No aux = inicio;
		while (aux != null) {
			aux = aux.getProx();
	...
	public static void inserir(int posicao, No inicio, String dado) {
		No novo = new No();
		novo.setDados(dado);
		No aux = new No();
		novo.setProx(inicio);
	...
	private static void transformarLSECircular(No inicio) {
		No aux = inicio;
		// ultima posicao
	...
	// Imprimir a lista ao contrário
	private static void imprimirContrario(No inicio) {
	...
	// TODO LIFE*
	private static void inserirPos(int posicao, No inicio) {
		No aux = inicio;
		for (int i = 0; i <= posicao; i++) {
			aux = aux.getProx();
	...
	// Inverter a Lista
	private static void inverter(No inicio) {
		// TODO Auto-generated method stub

	}

	private static void inserirFinal(String dado) {
		No no = new No();
		no.setDados(dado);
	...
	private static void imprimirPenUltimo(No inicio) {
		No aux = inicio;
